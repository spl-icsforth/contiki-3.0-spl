...
#include "dev/tmp102.h"  	//z1 temperature measurement
...
static void getappdata(){	
		...
		//uint16_t humm = sht11_sensor.value(SHT11_SENSOR_HUMIDITY);
		/* since we have z1 mote we need 
		to change those values. TEST! */
		msg.temp= tmp102_read_temp_raw();  // Reading from the sensor;
...
PROCESS_THREAD(sensing_process, ev,data)
{
	...
	// this seems a good place to initialize temperature sensor for z1
	//tzugarak
	tmp102_init();
	...
	// init the sensor	
	//tmp102_init();  -> if I put it here it seem's to do something like a reset
